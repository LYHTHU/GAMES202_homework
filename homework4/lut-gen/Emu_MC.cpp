...
float Fresnel(Vec3f i, Vec3f h, float R0) {
    float cos_theta = dot(i, h);
...
Vec3f IntegrateBRDF(Vec3f V, float roughness, float NdotV) {
    ...
    /// @param V : maybe wi?
    /// @param roughness
    /// @param NdotV : miu_o(sin(theta)); N dot wi; L(light)
    /// @return
    float A = 0.0;
    float B = 0.0;
    float C = 0.0;
    ...
    for (int i = 0; i < sample_count; i++) {
      // TODO: To calculate (fr * ni) / p_o here
        Vec3f wo = sampleList.directions[i];
        float pdf = sampleList.PDFs[i];
        Vec3f h = normalize(V + wo);
        float NdotO = dot(wo, N);
        float F = Fresnel(V, h, 1.0);
        // float G = GeometrySmith(roughness, dot(h, wo), dot(h, V));
        float G = GeometrySmith(roughness, NdotV, NdotO);
        float D = DistributionGGX(N, h, roughness);
        float fr = F * G * D / (4. * NdotV * NdotO);
        float value = fr * NdotO / pdf;
        A += value;
        B += value;
        C += value;
    }
