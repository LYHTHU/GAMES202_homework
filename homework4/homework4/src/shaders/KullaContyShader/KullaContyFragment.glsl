...
float GeometrySmith(vec3 N, vec3 V, vec3 L, float roughness)
{
    ...
    float ggx1 = GeometrySchlickGGX(max(dot(N, V), 0.), roughness);
    float ggx2 = GeometrySchlickGGX(max(dot(N, L), 0.), roughness);
    return ggx1 * ggx2;
}
...
vec3 MultiScatterBRDF(float NdotL, float NdotV)
{
  ...
  vec3 F_ms = (1. - E_o) * (1. - E_i) / (PI * (1. - E_avg));
  vec3 F_add = F_avg * E_avg / (1. - F_avg * (1. - E_avg));
  return F_add * F_ms;
}
