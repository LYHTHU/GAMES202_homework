                ...
                for (int x = 0; x < width; x++)
                {
                    ...
                    for (int l = 0; l < SHOrder+1; ++l) {
                        for (int m = -l; m < l+1; ++m) {
                            ...
                            float sh_coef = sh::EvalSH(l, m, dir.cast<double>().normalized());
                            SHCoeffiecents[coeffIndex] += Le * sh_coef * dw;
                        }
                    }
                }
            }
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++)
        {
            ...
            auto shFunc = [&](double phi, double theta) -> double {
                ...
                std::vector<Eigen::Array3f> result(SHCoeffLength);
                ...
                else
                {
                    ...
                    double is_intersected = scene->rayIntersect(ray);
                    if (!is_intersected) {
                        float H = wi.dot(n);
                        if (H > 0.) {
                            return H;
